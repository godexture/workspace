...
| F22 | multi-input filter が goroutine の到着順で入力を選ぶ経路を持ち、mixer、sidechain、A/V sync、EOF の意味が scheduler timing に依存し得る。 | [runtime](runtime.md)、[performance](performance.md)。M5 で generic fan-in の lifecycle/ownership を整理し、M7-1 で `SerialFanIn` の callback 同期直列化と input ordinal を確定した。`flow.Direct` を宣言した port を単一 routed producer が駆動する direct island では call 順を physical order として扱い、宣言のない generic Serial 構成の cross-track physical interleave は契約しない。MP4 correctness/exact は per-track の `Packet.Sequence`、PTS/DTS/duration、sample table と、demux が sample offset の merge で作る格納順の保持を基準にする（F60）。入力が持たない physical order を新規に組む consumer は、execution signature と別 ordered policy/backpressure を必要とする。 | M5/M7 |
...
| F56 | runtime が `SerialFanIn` input へ source buffer を挿入し、単一 routed producer を route ごとの drain task へ置き換えていた。callback は直列化されるが到着順は task 間の競合で決まるため、到着順を出力構造へ変換する component が非決定的に失敗する。M7 では multi-track MP4 remux が GOMAXPROCS=1 でほぼ常に失敗した。設計文書はこの構成を前提と書いていたが、それを検査する主体がなかった。 | [M7-0 C03/C09/C11](m7-0.md)、[runtime](runtime.md#execution-island)。**完了（M7-5）**: serial input は buffer せず、`flow.Direct` で単一 routed producer の island を component が要求し、満たせない topology は Planning error になる。回帰は `TestMP4RemuxKeepsTrackOrderAcrossManySamples` と `TestDirectManyInputRejectsUnsatisfiableProducers` が固定する | M7 |
| F57 | MP4 conformance corpus の multi-track fixture が全て 1 track 1 sample、多 sample fixture が全て single track で、実際の movie の形（複数 track × 複数 sample）を通る gate が一件も無かった。F56 はこの穴に隠れていた。 | [quality](quality.md)、[fixtures](fixtures.md)。**完了（M7-5）**: fixture builder に sample 数を持たせ、`mp4ManySampleTwoTrackFixture` を corpus へ追加した。以後 MP4 の新 gate は「track 数 × sample 数」の両軸を持つ fixture で書く | M7 |
| F58 | MP4 の `mdia`/`minf`/`stbl` 未知 child box を Inspect が一律 `ErrUnsupported` にするため、`elng` や `sgpd`/`sbgp`/`cslg` を持つ実ファイルが remux も demux/decode もできない。保存優先の fail closed としては正しいが、実 codec が増えると受理範囲の狭さが利用者に見える。 | [capability B11](capability.md)、[media](media.md)。copy 経路に限って通す box 群と、decode 経路で fail closed を維持する box 群を分けて決める | M8 |
| F59 | MP4 fixture builder が `stsc` を常に 1 entry、`stco` を常に 1 offset で書いていたため、全 corpus が「1 track = 1 chunk」だった。chunk-offset table の再構成、track を跨いで interleave された実 movie の形、journal の複数 entry と page またぎが end-to-end で一度も通っていない。M7-C12 の resource gate も 1k/1M どちらも chunk 数 1 のため、journal が実際に増える軸（chunk 数）を検査していなかった。F57 と同じ「fixture が軸を跨がない」型の穴である。 | [fixtures](fixtures.md#tier-2-conformance)、[quality](quality.md)。**完了（M7-6）**: fixture builder が track 数 × sample 数 × chunk 数の三軸を持ち、`mp4FixtureInterleaved` が chunk を交互に配置する。回帰は `TestMP4RemuxRebuildsInterleavedChunkTables` と `TestMP4RemuxJournalGrowsOnDiskNotInMemory` が固定する | M7 |
| F60 | MP4 demux が track ごとに全 sample を読み切ってから次の track へ進むため、mux は mdat を track 順に書き直し、interleave された movie が出力で de-interleave されていた（全 video → 全 audio）。sample table は正しいので再生自体は成立するが、player は同期を保つために file 全長を跨いで seek することになり、progressive/streaming 再生が壊れる。「保存優先の remux」が唯一保存しないものが、実ファイルの物理配置だった。当初これは「実際の interleave を保つには ordered policy と cross-route backpressure が要る」として M9 へ延期されていたが、それは cross-track の *timestamp* 順を作る場合の話であり、source の格納順は同一 file 内の byte offset という全順序として既に入力にある。 | [capability](capability.md)、[M7-0 C03/C11](m7-0.md)。**完了（M7-6）**: demux は per-track cursor を sample offset で merge して格納順に emit する（cross-track の timestamp 比較なし、単一 routed producer のまま、backpressure 不要）。mux は per-track cursor を持ち、chunk offset を track ごとの journal 領域へ `Scratch.WriteAt` で記録する。全 track を保つ remux は入力と byte 一致する。回帰は `TestMP4RemuxKeepsTheStoredSampleOrder` と `TestMP4RemuxRebuildsInterleavedChunkTables` が固定する | M7 |
