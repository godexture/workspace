...
| F3 | conversion/routing が単一の主 audio stream と固定 Packet/Frame 経路を前提にし、複数 input/output、video、subtitle、data、attachment、program を表現できない。 | foundation/cut は **完了（M5）**。static `Many` port、ordered repeated descriptors、typed Router、`Joiner + MergeFanIn` の最初の production consumer と残る完了条件は [M7-0 contract](m7-0.md) の M7-C01〜M7-C11 で追跡する | M3/M7 |
