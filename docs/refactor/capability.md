...
| multi-value の順序保持 | WAVE で固定済み | 維持 | M3/M7 | Document の順序 test |
| 重複 key の保持 | WAVE で固定済み | 維持 | M3/M7 | Document の重複 test |
| 未知 payload の raw 保持 | WAVE で固定済み | 維持 | M3/M7 | RawBlock の roundtrip |
...
| 機能 | 現状 | 決める時期 |
|---|---|---|
| codec 省略時に decoder/encoder を開く挙動 | M0 で現行挙動として固定済み | M7（[C4](decisions.md) の copy 優先と衝突するため） |
...
正式 release 前のため差異そのものは許容するが、**差異を残さないまま旧経路を削除しない**。旧実装は削除されるまで、差異が意図的か bug かを判断するための diagnostic として利用できる。
...
- M11 で旧経路を削除する前に、`未定` の行が残っていないことを確認する。
