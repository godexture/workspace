...
| B10 | MP4 remux は mdat を track 順に書き直すため、入力が interleave されていた movie は出力で de-interleave される（全 video → 全 audio）。chunk offset は patch されるので再生も per-track seek も成立するが、streaming/progressive 再生の品質は入力より落ちる | mux は `flow.Direct` の emit call 順を physical order として扱い、cross-track の global DTS interleave を作らない。実際の interleave を保つには ordered policy と cross-route backpressure が要り、その両方が実 consumer を必要とする。M7 の correctness/exact は track ordinal、`Packet.Sequence`、PTS/DTS/duration、per-track sample table で判定し、physical interleave の変更を semantic loss と扱わない | M7（記録）／M9（interleave policy） |
