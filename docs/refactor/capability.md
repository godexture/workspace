...
| B7 | WAVE の payload と入力由来 chunk は順序・padding・byte 列を exact に復元するが、file 全体の byte 一致は保証しない | [media](media.md#m6-完了条件) のとおり、全出力の先頭に `ds64` と同じ 28 byte payload を持つ `JUNK` chunk（header 込み 36 byte）を予約する。header 長を固定して `RIFF`/`data` size の後追い patch と 4 GiB 超での RF64 化を成立させるためである | M6 |
