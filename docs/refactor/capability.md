...
| B11 | MP4 の `mdia`/`minf`/`stbl` の未知 child box は Inspect で `ErrUnsupported` になり、その movie は remux も demux/decode もできない | 保存優先の既定で未知 box を黙って落とさないための fail closed である。ただし `elng`、`sgpd`/`sbgp`/`cslg`/`saiz`/`saio`/`subs` を持つ実ファイルは読めない。実 codec が増える M8 で、copy 経路に限って通す box 群を決める | M7（記録）／M8（受理範囲） |
| B12 | MP4 は sample table が指さない byte が `mdat` にある movie を `ErrUnsupported` で拒否する（`totalSampleBytes != mdat payload size`）。chunk 間 padding や未参照領域を持つ実ファイルが該当する | remux は mdat payload を sample 単位で書き直すため、どの sample にも属さない byte は行き先を決められない。黙って落とすと保存優先に反し、そのまま残すと offset が壊れる。B11 と同じ fail closed の系統で、受理範囲を広げるなら「未参照領域を source range として保持する piece」を layout へ足す判断が要る | M7（記録）／M8（受理範囲） |
| B13 | 32-bit `stco` を持つ track がある movie は、出力 payload の終端が 4 GiB を超えると Compile で `ErrUnsupported` になる。個々の chunk offset が 4 GiB 未満に収まる movie も拒否する | 各 chunk は必ず payload の内側に入るので payload 終端が上界になる。厳密判定は per-chunk の累積 sample size を要し、それが分かるのは mdat を書き終えた後である。入力の `stco` も 32-bit で、出力の moov は縮むことしかないため、これに該当する入力は実質的に「入力自身が 4 GiB を扱えていない」movie に限られる | M7 |
| B14 | MP4 を出力 format にできるのは、同じ MP4 を入力に持つ remux だけである。WAVE → MP4 のように MP4 を新規生成する経路は存在しない | M7 の MP4 mux は inspected movie を必須にし、入力 descriptor の stream ID・time base・sample entry tag・metadata が inspection と完全一致することを要求する preserving remuxer である。moov を descriptor から組み立てる汎用 mux は、sample table を出力側で新規構築する別実装であり、M7 の保存優先 slice には含めない。公式 family の一つが「入力が同じ format の時だけ出力できる」状態のままなので、担当を決めて解消する | M7（記録）／M9（汎用 mux） |
| B15 | `sidx`/`ssix`/`mfra`/`tfra`/`iloc` のように sample table の外へ byte offset を記録する movie は、出力が source を再現する選択（全 track を保つ remux）だけ受理する。track を落とす選択は従来どおり `ErrUnsupported` | 当初は全選択も拒否していたが、それは remux が mdat を track 順に書き直していた頃の前提である。格納順を保つようになった今、全 track remux は入力を byte 単位で再現するので外部 offset は stale にならない。ただしこれを「前提」にはせず検証する: layout が全 track・同一 payload 位置・同一総 size を満たすかを Compile で確認し、各 sample が読んだ位置と同じ位置へ書かれるかを Process で確認する。判定が plugin 内で閉じるため、runtime の direct island を将来どう扱っても受理の正しさは影響を受けない | M7 |
