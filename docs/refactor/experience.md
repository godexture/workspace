...
- **`cmd/godec` が新 Host の上で動く。** WAVE/PCM の範囲に限り、入力と出力を指定した変換が公式 binary から実行できる。`cli.Run(ctx, h, args)` の形で `Host` を注入され、CLI layer に planner、registry、plugin factory を持たない。この時点の CLI が持つのは入出力指定、`Plan` preview、progress 表示、cancel、exit code の分類だけでよい。`cli.Run` は分類した `ExitCode` と、その裏で起きた独立な failure をすべて join した error の両方を返す。非 zero code は「何かが失敗した」しか言えず、broken pipe、cleanup 失敗、元の runtime 失敗を区別できない。process wrapper である `cmd/godec` だけが code を `os.Exit` へ写す。
- **利用者が拡張の種類を知らなくてよい。** 第三者が Access Provider を含む plugin を配っても、利用者が書くのは `standard.Set().Add(acme.Plugin())` であり、合成 API を種類ごとに使い分けない。公式 composition は `standard.NewHost()` の一行で足りる。Endpoint も同じ `plugin.Set` の合成形で表せるが、実際の利用と conformance は M9 が担当する。合成の入口が拡張の分類を利用者へ漏らしていないことを、第三者相当 Provider plugin の受け入れ test で確認する。
