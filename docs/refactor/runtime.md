...
**Host が新しい journal を開いてよいのは、その journal が属していた goroutine が確実に戻ったと言える時だけ**である。source の `Finish` と join の `Finish` はこれを満たす: source は `WaitSources` が、join は barrier 自身が `<-s.done` を待つことで、呼び出し前に goroutine の終了を確認している。bounded edge の barrier (`Queue.WaitIdle`) はこれを満たさない —— ring が空であることしか約束せず、goroutine が戻ったことは約束しない。これは欠陥ではなく設計である: Finalize が生成する遅延 Flush 出力を、Quiesce の後も同じ queue で受け取れるように、drain task はわざと生き続ける。したがって bounded edge の下流 close は、Host が `Finish` から新しい journal を開いて行うものではなく、drain task 自身が EOF を見た時に**自分の journal の続きとして**行う。`Execution.Finish` は `namedTask.chain` が有効な entry（source と join）にだけ Host 主導の Flush journal を開き、bounded edge の namedTask（`chain` を持たない）はこの経路に触れない。触れれば、まだ生きているかもしれない drain task の journal を別 goroutine から読み書きすることになる。

「自分の journal の続き」であって「Run journal の続き」ではないのは、`Operation` が `Scope` に固定された値ではなく、**その時点で記録される failure に付くラベル**だからである。`Scope.EnterOperation(operation) Operation` は、同じ journal・同じ writer のまま、これから記録する failure の分類だけを書き換えて、直前のラベルを返す。drain task は EOF を見て `target.close(ctx)` を呼ぶ直前に `EnterOperation(journal.Flush)` する。別 goroutine を起こさず、別 journal も開かないので、cross-goroutine の同期問題を再導入しない。ラベルを戻し忘れても安全なのは、この時点で ring が証拠上空だからである（`Queue.Pop` は `count == 0 && closed` の時だけ `io.EOF` を返し、`Close` は不可逆なので、以後この goroutine が Run 種別の failure を記録することはない）。

`Operation` はこの通り記録時点で変わりうるため、event identity には使わない。`EventID` は `Task`・`Attempt`・`Seq` の組であり、`Attempt` は `journal.New` が呼ばれるたびに process 全体で採番される、Scope オブジェクト一つに対して一つの値である。同じ Task に対して Run 用と Flush 用の Scope オブジェクトが別々に開かれても（`namedTask.flush` が新しい Scope を開く場合）、両者の `Attempt` は異なるため、それぞれの `Seq` が 1 から始まっても衝突しない。`Operation` は `Failure` に個別に載る phase 写像用の metadata であり、identity には一切関与しない。一つの Outcome の中で Primary と Cleanup が異なる `Operation` を持ちうるので、`Outcome` 自身は `Operation` を持たず、Host は各 `Failure.Operation` を見て phase を決める。

boundary（task の goroutine、または Host が代わりに行う Finish の hand-off）はどちらも `journal.Capture(scope, work)` を通して journal を組み立てる。`Capture` は `work` の panic を recover して `scope.Panicked` に記録し、成功時は `scope.Fail(work())` した上で `scope.Seal()` を返す — defer の中で `outcome` を named return として代入するので、panic 経路でも必ず `Seal()` まで届く。task 自身の Run と、Host が `namedTask.flush` で開く Flush はどちらもこの一つの helper を通るため、「Host 主導の hand-off だけ panic で journal を失う」という非対称性が無い。`Execution.Finish` 自身も、この hand-off の結果（`Outcome` の列）だけを返し、それとは別の `error` を並行して返すことはしない。Finish の error も panic も `Outcome.Primary` に入り、Host は他の lifecycle operation と同じ経路でそれを読む。

cancellation cause にも event の出自を持たせる。`journal.Cause{Event EventID, Err error}` は `Outcome.Cause()` が返す値で、`task.Group` はこれを `context.CancelCauseFunc` へ渡す。ある task が `ctx.Done()`/`context.Cause(ctx)` をそのまま自分の error として返すパターン（この codebase 全体で使われている）は、`context.Cause(ctx)` を経由するだけで自動的に同じ `*Cause` 値を運ぶため、`errors.Is` によるピア間の cancellation echo 判定（`task.Group` 内の `cancellationEcho`）は変更なしで動く。Host 側の echo 判定（`acceptTaskFailure`）は、`errors.As` で `context.Cause(r.ctx)` から `*journal.Cause` を取り出し、`cause.Event == value.ID` で比較する。同じ sentinel error を返した二つの独立した failure が、値の一致だけで一方を握り潰す、という誤判定が構造的に起こらない。ただし、この判定はあくまで「Host が既に `Result.Primary` を確定させた後」だけに効く（`r.result.Primary != nil` を先に見る）。確定前の最初の観測は、それが cancellation の原因そのものであっても、`Result.Primary` になる資格を失わない。
