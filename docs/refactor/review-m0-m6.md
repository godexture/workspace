...
これにより、buffered edge を経由した plugin failure の phase は本来あるべき `FlushPhase` ではなく `RunPhase` のまま残る — direct chain の Flush failure が `FlushPhase` になるのとは topology 依存で異なる。今回は「Flush failure が消える」「journal を別 goroutine から読んで競合する」の二つを優先して閉じ、この phase の不一致は残課題とする。
