...
- **component は実行可能とは限らない。** 合成時の拡張がすべて component に乗る以上、data path を持たない拡張も component として宣言できなければならない。M5 時点の `plugin.Component` は typed `Spec` を必須とし、既定 shape も Spec 経由でしか解決できないため、metadata Encoding のように Inspect 中に trait から直接呼ばれるだけで `Open` も port も持たない control-plane 拡張を宣言できない。したがって component は「**Spec と port を持つ実行 component**」と「**trait だけを持つ control-plane component**」の両方を表せるようにする。不要な flow schema、operator、`Open` を捏造しない。
- **ただし validity を緩めない。** component が有効なのは Spec と port を持つ場合、または trait を一つ以上持つ場合であり、**どちらも持たない component は従来どおり拒否する**。ここを緩めると consumer の無い宣言を黙って登録できてしまう。
- **trait は port shape を要求するかどうかを自分で表明する。** Access source/sink は 0-in/1-out と 1-in/0-out、Format read/write は byte port を要求し、Metadata Encoding は shape を要求しない。この区別を trait 側に持たせないと、catalog は「encoding が正当に port を持たない」ことと「access source が port を書き忘れた」ことを区別できない。
- **control-plane component は graph node になれない。** catalog には載り、Host が carrier ID や scheme から解決できるが、solver の候補にならず、Job node として指定された場合は専用の構造化 diagnostic で拒否する。Spec 欠落や shape 不整合として分かりにくく失敗させない。`ComponentView` は実行可能かどうかを surface が判別できる形で示し、catalog 駆動の editor が挿入候補として提示しないようにする。
- **`Compile` は cancellation を受け取るが、値は受け取らない。** metadata chunk は sink へ渡す payload であり、payload の grant は `Compile` で宣言する。したがって marshalled size を知るために `Compile` から Metadata Encoding の `Marshal` を呼ぶ必要があり、`context.Background()` で呼ぶと planning の cancellation と budget を無視する。`plugin.CompileContext` に context を足し、solver が requested node、gap 再評価、bridge candidate のすべての `Compile` 呼び出しへ伝播する。ただし返すのは `Done`/`Err`/`Deadline` だけを転送し `Value` が常に nil を返す context とする。素の `context.Context` を渡すと第三者が値を読んで出力を変えられ、[planner](planner.md#m4-完了条件) が要求する「同じ入力なら同じ Plan fingerprint」が静かに崩れる。context は purity の定義を変えない。
- **purity を test で固定する。** cancellation が requested component、自動挿入 component、Metadata Marshal まで届くことに加え、**deadline だけが異なる二つの context で同じ Job を compile したとき同じ Plan fingerprint になる**ことを検査する。伝播だけを見る test では「context は cancellation 専用」という規則が honor system のままになる。`SuggestContext` は空のままとする。`Suggest` は bounded で I/O を持たない契約であり、context を渡す理由がない。
- **definition が自分に属する declaration を運ぶ。** M5 時点の `plugin.Definition` は component しか持たず、codec/metadata Binding と vocabulary の key 宣言は `Set.AddDeclaration` で別に足す必要がある。そのままでは第三者が Format・Codec・Metadata Encoding をまとめても Binding が `Set` へ運ばれず、`standard.Set().Add(acme.Plugin())` だけで合成が完結しない。definition に owned declaration を持たせ、`Set.Add` が component と一緒に運ぶ。`Set.AddDeclaration` は composition root 用として残す。Binding を trait へ作り替えない。Binding は key と component identity の**関係**であり、一つの component の性質ではないためである。
- **owned declaration の寿命は definition と一致する。** `Remove` と `OverridePlugin` は owned declaration も一緒に扱う。component を一つだけ外した結果として owned declaration の target が消えた場合は、declaration を黙って落とさず catalog の target 実在検査で失敗させ、外した component と所有 definition の両方を diagnostic に出す。`WithDeclarations` は新しい `Definition` を返し、不正・重複の declaration は捨てずに problem として保持する。
- **owned declaration の target を definition 内へ制限しない。** 公式 family が family 間 Binding を composition root へ置くのは慣習ではなく構造上の理由で、他 family の component identity を参照するには その family を import する必要があり、公式 plugin 間の直接 import 禁止に反するためである。第三者は公式 family を import してよいので、第三者の container plugin は公式 codec への Binding を own できる。ここを制限すると、その正当な場合が `AddDeclaration` に戻り、利用者が拡張の種類を知らなくてよいという条件が壊れる。合成を守るのは既存の衝突検出である。
- **definition は自分が依存する vocabulary declaration も own する。** そうしないと `plugin.NewSet(acme.Plugin())` 単体で宣言が欠ける。同一内容の重複宣言は既に許容されている。
