...
- **boundary component が port schema を満たすことを composition で検証する。** `access.Source` の出力 port、`access.Sink` の入力 port、Format read trait の入力 port、Format write trait の出力 port がいずれも `access.Bytes()` であることを `internal/catalog` の trait 走査が検査する。Provider と Format が実際に接続可能であることを Host 構築時に保証し、graph 検証まで持ち越さない。
