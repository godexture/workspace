...
- RIFF INFO の Format owner は carrier が無ければ `Absent`、各 carrier を bound Encoding が opaque block を残さず完全な `Document` に parse できれば `Available`、carrier はあるが完全な Document を作れなければ `Unavailable` として `AssetScope` の attachment を descriptor へ投影する。builtin RIFF INFO parser の未知 child・不正値・構造不全と任意の bound Encoding の content parse failure は `Unavailable` である。semantic authority は bound Encoding の `Parse` が返す `Document` であり、owner inspection は Unavailable attachment と検証済み source range を別々に保持する。同じ inspection state の exact range handoff だけを許し、custom binding の inspected edit は rewrite layout 不在として拒否する。budget、I/O、truncation、cancel、runtime overflow は hard error とする。
- RIFF INFO の marshal は document の編集を検出して扱いを分ける。未変更 document は元 carrier の bytes をそのまま返す。編集時は source の semantic slot と target entry を wire/document 順に一度ずつ対応させ、同じ位置の未変更 child は raw bytes と padding を、未知 child は元の位置と bytes を保持する。変更・移動した semantic child は canonical に再 encode し、削除分は除去、追加分は末尾へ書く。この処理は source と target の合計件数に対して linear であり、semantic edit を raw のまま黙って返さない。
