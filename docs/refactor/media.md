...
- RIFF INFO の Format owner は carrier が無ければ `Absent`、各 carrier を bound Encoding が opaque block を残さず完全な `Document` に parse できれば `Available`、carrier はあるが完全な Document を作れなければ `Unavailable` として descriptor へ投影する。builtin RIFF INFO parser の未知 child・不正値・構造不全と任意の bound Encoding の content parse failure は `Unavailable` である。semantic authority は bound Encoding の `Parse` が返す `Document` であり、owner inspection は Unavailable attachment と検証済み source range を別々に保持する。同じ inspection state の exact range handoff だけを許し、custom binding の inspected edit は rewrite layout 不在として拒否する。budget、I/O、truncation、cancel、runtime overflow は hard error とする。
