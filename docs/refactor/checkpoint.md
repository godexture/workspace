...
> 実装進捗: **7 / 12 マイルストーン完了（M0〜M6）**。M6 の再完了と最終 verification は 2026-08-17 に記録済み。M7-0 を static multi-stream vertical slice へ是正し、M7-1 の static execution foundation（private logical→physical connections、typed Router、selected `Batch`、`Joiner + MergeFanIn`）を完了した。次は unfragmented MP4 vertical slice である。
...
| M7 | 進行中 | [M7-0 contract](m7-0.md) を static `Many` topology、ordered repeated descriptors、Router、`Joiner + MergeFanIn`、unfragmented RandomRead+StableSize/RandomWrite MP4 vertical slice へ是正した。M7-1 の static execution foundation は完了し、per-connection `Span`、Zip tolerance、mixed-base DTS merge、ownership/cancel/EOF/Flush の runtime contract を固定した。次は M7-2 の unfragmented MP4 reader/writer/remux。 |
...
- M7-0 は dynamic per-track Shape と過剰な同時 surface 追加を撤回し、static `Spec.Ports`、logical `Many` edge、default/exact mapping、unfragmented MP4 remux を M7-1〜M7-4 へ分解した。M7-1 は logical edge を descriptor ごとの private connection へ展開し、typed Router と selected `Batch`/`MergeFanIn` を実装した。Merge は全 live head を待ち、mixed time base の DTS を exact に比較し、同時値を input ordinal/FIFO で安定化する。queue の per-input `Span` と Zip tolerance は分離し、cancel/failure 時の held/queued item、正常 EOF 後の drain/Flush を runtime が所有する。次は M7-2 の unfragmented MP4 reader/writer/remux であり、fragmented/spool、rich selector、generic loss DTO/`ilst`、seek は実 consumer の M8/M9 へ延期した。M7 の次作業と traceability は [M7-0](m7-0.md) を正本とする。
