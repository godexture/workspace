...
> 実装進捗: **7 / 12 マイルストーン完了（M0〜M6）**。M6 の再完了と最終 verification は 2026-08-17 に記録済み。M7-0 を static multi-stream vertical slice へ是正し、M7-1 は private logical→physical connections と typed Router を実装済みで進行中。次は selected `Batch`/`Joiner + SerialFanIn` の serial execution である。
...
| M7 | 進行中 | [M7-0 contract](m7-0.md) を static `Many` topology、ordered repeated descriptors、typed Router、callback の同期直列化と input ordinal を持つ `SerialFanIn`、unfragmented RandomRead+StableSize/RandomWrite MP4 vertical slice へ是正した。M7-1 の private logical→physical connections と typed Router は実装済みだが、M7-1 全体は未完了で、次は selected `Batch`/`Joiner + SerialFanIn` serial execution。Serial input buffer のない direct な single synchronous execution island でだけ emit call 順を physical order と扱い、buffer/fan-out/concurrent producer の cross-track interleave は契約しない。M7-2 は explicit 1 input/1 output MP4 graph、M7-3 は standard automatic Many terminal/mapping を担当する。 |
