...
| M8 | 進行中 | M8-1 の typed sample vocabulary を完成した。coding/packing/endian を直交させ、`Layout` を channel 位置集合にし、decoded frame の canonical schema を四つに固定した。linear PCM は全 coding と任意 channel 数を、WAVE は 8/24/32 bit・float・channel mask を、MP4 は `raw `/`in24`/`in32`/`fl32`/`fl64` を扱う。schema 間 converter を `plugin/audio` に置き、WAVE mux の coding 要求を最初の consumer にした。次は M8-2 の PCM family completion。 |
