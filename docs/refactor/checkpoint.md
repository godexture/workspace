...
| M2 | 完了 | `diagnostic`、typed `config`、marker identity/immutable `plugin.Set`、検証済み `internal/catalog`、`host.New` と review 指摘 1〜10 の是正を完了し、対象 race、build、全体 `--simd` gate、構造化 codec の roundtrip を green にした。 |
| M3 | 未着手 | — |
| M4 | 未着手 | — |
| M5 | 未着手 | — |
| M6 | 未着手 | — |
