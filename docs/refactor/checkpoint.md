...
> 実装進捗: **7 / 12 マイルストーン完了（M0〜M6）**。M6 の再完了と最終 verification は 2026-08-17 に記録済み。M7-0 を static multi-stream vertical slice へ是正し、M7-1 は private logical→physical connections、typed Router、selected `Batch`/`Joiner + SerialFanIn` の serial execution を実装・レビュー済みで完了。M7-2 は narrow same-format immutable `Inspection` handoff を先に行い、その後 MP4 parser/Inspect へ進む。
...
| M7 | 進行中 | [M7-0 contract](m7-0.md) を static `Many` topology、ordered repeated descriptors、typed Router、callback の同期直列化と input ordinal を持つ `SerialFanIn`、unfragmented RandomRead+StableSize/RandomWrite MP4 vertical slice へ是正した。M7-1 の private logical→physical connections、typed Router、selected `Batch`/`Joiner + SerialFanIn` serial execution は実装・レビュー済みで完了。M7-2 は narrow same-format immutable `Inspection` handoff を最初に行い、その後 MP4 parser/Inspect を含む explicit 1 input/1 output MP4 graph へ進む。Serial input buffer のない direct な single synchronous execution island でだけ emit call 順を physical order と扱い、buffer/fan-out/concurrent producer の cross-track interleave は契約しない。M7-3 は standard automatic Many terminal/mapping を担当する。 |
...
- M7-0 は dynamic per-track Shape と過剰な同時 surface 追加を撤回し、static `Spec.Ports`、logical `Many` edge、callback の同期直列化と input ordinal を持つ `SerialFanIn`、explicit MP4 graph、default/exact mapping、unfragmented MP4 remux を M7-1〜M7-4 へ分解した。M7-1 は実装・レビュー済みで完了し、M7-2 は narrow same-format immutable `Inspection` handoff を先に行って MP4 parser/Inspect へ進む。MP4 correctness/exact は track ordinal、`Packet.Sequence`、PTS/DTS/duration、per-track sample table を基準とし、physical interleave の変更を semantic loss としない。buffer/fan-out/concurrent producer を理由に generic `SerialFanIn` を Compile で reject せず、Stable/byte reproducibility が必要な将来 consumer は execution signature と別 ordered policy/backpressure を要求する。fragmented/spool、rich selector、generic loss DTO/`ilst`、seek、別 container の time-ordered fan-in は実 consumer と backpressure 設計が揃うまで延期した。M7 の次作業と traceability は [M7-0](m7-0.md) を正本とする。
