...
- M7-3 の実装で確定した二つの product 判断を記録する。(1) `sidx`/`ssix`/`mfra`/`tfra`/`iloc` のように sample table の外へ byte offset を記録する box を持つ movie は、track subset だけでなく全 track の remux も拒否する。remux は mdat payload を sample 単位で書き直すため、これらの offset は stale になり得る。M7-6 で格納順を保つようにした結果、全 track を保つ remux は入力と byte 一致するので、その場合に限れば offset は stale にならない。受理範囲を広げるかは「byte identity が preserve-all の契約か、たまたま成立している性質か」を決める判断であり、[B14](capability.md) の汎用 mux と同じ M9 で扱う。(2) `tref` の target track ID は inspection に保持しない。track model を slice 無しで保つ代わりに、`tref` を持つ track を含む subset は fail closed とする。全 track を保つ remux は `trak` を verbatim に複製するため影響しない。
