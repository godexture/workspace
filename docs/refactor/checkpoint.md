...
| M3 | 進行中 | 3 単位に分けた。M3-1（docs gate、media/access contract、marker identity、一般化 declaration、flow ownership、static shape/Open、packet 段付き walking skeleton、erased descriptor factory）を [task/m3-1-fix.md](task/m3-1-fix.md) の項目 1〜16 まで含めて完了し、M3-2 の metadata model/Binding、共通 `tag` vocabulary、raw-preserving metadata skeleton まで実装した。次は M3-3（残る access/media 拡張点）。schema の一致判定は `Descriptor` の比較ではなく `Identity()` が担い、非比較性を型で強制している。固有の完了条件は [media.md](media.md#m3-完了条件)、[access.md](access.md#m3-完了条件)、[scope.md](scope.md#m3-完了条件) を参照する。 |
| M4 | 未着手 | — |
| M5 | 未着手 | — |
| M6 | 未着手 | — |
| M7 | 未着手 | — |
...
- `host.Open(identity)` は Plan/Program/transaction を経ずに component を開く public API であり、walking skeleton のためだけに存在する。[runtime.md](runtime.md#plan-と-open-transaction) の lifecycle では M4/M5 が Prepare/Program 経由の Open に置き換えるため、M5 で削除する。制限は godoc にも書いた。
