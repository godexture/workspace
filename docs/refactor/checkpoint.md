...
| M3 | 進行中 | 3 単位に分けた。M3-1 の review 是正（docs gate、media/access contract、marker identity、一般化 declaration、flow ownership、static shape/Open、packet 段付き walking skeleton、erased descriptor factory）を実装し、対象 race、vet、build、docs-check、SIMD 全体 runner を通過した。次は M3-2（metadata model/Binding）、M3-3（残る access/media 拡張点）へ進む。固有の完了条件は [media.md](media.md#m3-完了条件)、[access.md](access.md#m3-完了条件)、[scope.md](scope.md#m3-完了条件) を参照する。 |
| M4 | 未着手 | — |
| M5 | 未着手 | — |
| M6 | 未着手 | — |
| M7 | 未着手 | — |
...
- erased descriptor から typed 実装を組み立てる機構は M3 の担当とする。一度 M5 へ先送りする判断をしたが撤回した。`flow.Port` が `schema.ID` しか持たないと declaration から erased 層へ `T` を運ぶ経路がなく、M5 が必ず `media/schema` と `flow` の public API を開け直すことになる。[media.md](media.md#m3-完了条件) はこれを M3 の完了条件として挙げており、[C2](decisions.md) と [C11](decisions.md) の拡張性主張が成立するかを決める箇所でもある。M3-1 で比較可能な `schema.Descriptor`、port 経由の factory、typed queue/fan-out consumer を実装し、queue policy だけを M5 へ残す。
- `host.Open(identity)` は Plan/Program/transaction を経ずに component を開く public API であり、walking skeleton のためだけに存在する。[runtime.md](runtime.md#plan-と-open-transaction) の lifecycle では M4/M5 が Prepare/Program 経由の Open に置き換えるため、M5 で削除する。
