...
> 実装進捗: **7 / 12 マイルストーン完了（M0〜M6）**。M6 の再完了と最終 verification は 2026-08-17 に記録済み。M7-0 を static multi-stream vertical slice へ是正し、M7-1 は private logical→physical connections、typed Router、selected `Batch`/`Joiner + SerialFanIn` の serial execution を実装・レビュー済みで完了。M7-2 は aggregate `ScratchMaxBytes` と node-local borrowed journal を実装し、単一 `mdat`・事前課金 Inspect・固定 page/lazy cursor を初期 MP4 parser contract として進める。
...
| M7 | 進行中 | [M7-0 contract](m7-0.md) を static `Many` topology、ordered repeated descriptors、typed Router、callback の同期直列化と input ordinal を持つ `SerialFanIn`、unfragmented RandomRead+StableSize/RandomWrite MP4 vertical slice へ是正した。M7-1 は実装・レビュー済みで完了。M7-2 の scratch foundation は固定 `Compiled.Scratch` node claim と selected output spool maxima を aggregate `ScratchMaxBytes`、Plan/fingerprint、Open-to-Close の borrowed `plugin.Scratch`、operator Close 後/session Close 前 cleanup へ接続済みである。初期 MP4 input は単一 `mdat` に限定し、Inspect の全 `ReadAt` を事前課金して固定 page で scan、Open lazy cursor は元 borrowed source を使う。複数 `mdat` は bounded disk index consumer まで unsupported とする。M7-3 は standard automatic Many terminal/mapping を担当する。 |
