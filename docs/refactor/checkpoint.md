...
> 実装進捗: **7 / 12 マイルストーン完了（M0〜M6）**。M6 の再完了と最終 verification は 2026-08-17 に記録済み。M7-0 を static multi-stream vertical slice へ是正し、M7-1 は private logical→physical connections、typed Router、selected `Batch`/`Joiner + SerialFanIn` の serial execution を実装・レビュー済みで完了。M7-2 は aggregate `ScratchMaxBytes`、node-local borrowed journal、単一 `mdat`・事前課金 Inspect・固定 page/lazy cursor に加え、RoutedReader runtime と provider carrier を生成しない MP4 direct reader を実装した。
...
| M7 | 進行中 | [M7-0 contract](m7-0.md) を static `Many` topology、ordered repeated descriptors、typed routed producer、callback の同期直列化と input ordinal を持つ `SerialFanIn`、unfragmented RandomRead+StableSize/RandomWrite MP4 vertical slice へ是正した。M7-1 の private connections、typed Router、SerialFanIn は実装・レビュー済みで完了。M7-2 の RoutedReader runtime と scratch foundation は固定 `Compiled.Scratch` node claim と selected output spool maxima を aggregate `ScratchMaxBytes`、Plan/fingerprint、Open-to-Close の borrowed `plugin.Scratch`、operator Close 後/session Close 前 cleanup へ接続済みである。MP4 input は direct RoutedReader として demux output を boundary anchor にし、provider carrier node/edge を作らず、一度 Acquire した Opening を Inspect/demux/same-format mux で共有する。Inspect の全 `ReadAt` は事前課金し、固定 page と current-track lazy cursor で constant RAM を保つ。payload-heavy Run の read bytes は source size の 1.25 倍以下に固定して carrier 全走査の回帰を検出する。複数 `mdat` は bounded disk index consumer まで unsupported とし、automatic graph は M7-3 まで明示 reject する。M7-3 は standard automatic Many terminal/mapping を担当する。 |
