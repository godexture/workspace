...
- 専門 testkit のうち Format、Codec/Parser、Access Provider を M6 に含める。ただし**実装と consumer を持つ contract だけを対象にする**。Access Provider は capability 選択、`Own`/`Borrow`、commit/abort までとし、**snapshot と retry は含めない**。`access.Session` は capability と close しか持たず、`Snapshot` は Host にも Plan にも到達しない値型、`Reopen` は操作を持たない capability 値であり、[scope](scope.md#m6-の-contract-分類) が再取得と retry を remote Provider の milestone へ割り当てている。ここで実装すると consumer の無い機構を作ることになる。Metadata Encoding は RIFF INFO が扱う範囲（parse/marshal、重複と順序、未知 raw）に限り、Mapping と loss は M7、Endpoint は M9 に残す。
