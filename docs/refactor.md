...
| M9 | 利用 surface の完成 | library、CLI、WASM、非 production demo が同じ Host/Job/Plan/Result を使う。M6 の最短経路を全 flag、versioned DTO、catalog 駆動 editor へ広げ、rich stream selector を確定する。stdin/stdout・remote/streaming consumer とともに pure fragmented/sequential MP4 と output boundary spool alternative を追加する。あわせて、descriptor から moov を組み立てる汎用 MP4 mux（[B14](refactor/capability.md)）を同じ作業として扱い、MP4 が入力と同じ movie の remux 以外でも出力 format になれるようにする。playback は typed Endpoint と専用 command へ分離し、M6 の拡張性 gate を surface 越しに再実行する | [surfaces](refactor/surfaces.md)、[web](refactor/web.md)、[experience](refactor/experience.md) |
...
M8 も同じ手順で、実装開始前に [M8-0 contract](refactor/m8-0.md) で公開境界、product semantics、M8-C01〜M8-C13 の traceability、sub-unit の依存順、固有完了条件を確定した。M8 の実装・完了判定ではこの文書の M8 行だけでなく、同 contract の M8-C01〜M8-C13 をすべて満たす。着手前監査で M0 baseline の MP3/FLAC demuxer が持つ `Seek` に呼び出し元が無かったことを確認した上で、graph operation としての seek を M8 で完成させる判断を維持した。requester は M8 の library API とし、surface への投影は M9 が担当する。
