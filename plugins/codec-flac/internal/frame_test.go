...
// TestDecodeFLACFrame_TruncatedDataReturnsError verifies that a frame whose
...
// decodeFLACFrame.
func TestDecodeFLACFrame_TruncatedDataReturnsError(t *testing.T) {
	...
	for cut := 1; cut < len(frameData); cut++ {
		...
		_, err := decodeFLACFrame(truncated, info)
		if err == nil {
			...
			if cut == len(frameData)-1 {
				t.Fatalf("decodeFLACFrame() with 1 byte missing succeeded, want an error")
			}
			continue
		}
	}
...
// TestDecodeFLACFrame_TruncatedFooterDetected checks the specific case of
...
func TestDecodeFLACFrame_TruncatedFooterDetected(t *testing.T) {
	data := mustDecodeHex(t, "664c6143800000221000100000000f00000f0ac442f0000000013e84b41807dc690307586a3dad1a2e0ffff869180000bf0358fd03128baa9a")
	info, err := streaminfo.Parse(data[8:42])
	if err != nil {
		t.Fatalf("streaminfo.Parse() error = %v", err)
	...
	_, err = decodeFLACFrame(truncated, info)
	if err == nil {
		t.Fatalf("decodeFLACFrame() with missing CRC-16 footer succeeded, want an error")
	}
	if !errors.Is(err, io.ErrUnexpectedEOF) {
		t.Fatalf("decodeFLACFrame() error = %v, want io.ErrUnexpectedEOF", err)
	}
}
