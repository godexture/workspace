...
type SpeedConfig struct {
	Factor float64   `name:"factor" help:"Playback retime multiplier (e.g. 2 for double retime, 0.5 for half); pitch shifts with retime"`
...
func (c SpeedConfig) Validate() error {
	if !finite(c.Factor) || c.Factor <= 0 {
		return fmt.Errorf("retime factor must be finite and positive")
	}
	if !c.Mode.Valid() {
