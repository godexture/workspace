...
func TestRIFFInfoEncodingAppendsEditsWithoutDiscardingOriginalBytes(t *testing.T) {
	...
	encoded, err := resolver.Marshal(t.Context(), RIFFInfo(), "list-0", edited)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRIFFInfoEncodingRejectsForeignChildProvenance(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			_, err = resolver.Marshal(t.Context(), RIFFInfo(), "list-0", edited)
...
func TestRIFFInfoEncodingRespectsDuplicateDocumentOrder(t *testing.T) {
	...
	encoded, err := resolver.Marshal(t.Context(), RIFFInfo(), "list-0", reordered)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRIFFInfoEncodingDoesNotSilentlyReturnRawForReplacement(t *testing.T) {
	...
	encoded, err := resolver.Marshal(t.Context(), RIFFInfo(), "list-0", replaced)
	if err != nil {
		t.Fatal(err)
	}
