...
func generate(output, packageName, typeName, optionName, configName, resolvedType, defaultExpr, presetFunc, presetNormalizer, extraImport string, emitConfig bool, structType *ast.StructType, source *ast.File) {
	...
	imports["optional"] = "github.com/godexture/sdk/optional"
	usedImports := map[string]string{}
	var body bytes.Buffer
	if emitConfig || configName != typeName {
		...
		for _, field := range structType.Fields.List {
			...
			usedImports["optional"] = imports["optional"]
			fmt.Fprintf(&body, "\t%s optional.Optional[%s]\n", fieldName, value.String())
		}
		body.WriteString("}\n\n")
	}
	fmt.Fprintf(&body, "type %s func(*%s)\n\nfunc New%s(options ...%s) %s {\n\tvar config %s\n\tfor _, option := range options {\n\t\toption(&config)\n\t}\n\treturn config\n}\n", optionName, configName, configName, optionName, configName, configName)
	...
	for _, field := range structType.Fields.List {
		...
		fmt.Fprintf(&body, "\nfunc With%s(v %s) %s {\n\treturn func(c *%s) {\n", fieldName, value.String(), optionName, configName)
		usedImports["optional"] = imports["optional"]
		fmt.Fprintf(&body, "\t\tc.%s = optional.Some(v)\n", fieldName)
		body.WriteString("\t}\n}\n")
	}
	if (resolvedType == "") && (defaultExpr != "") {
	...
	if resolvedType != "" {
		fmt.Fprintf(&body, "\nfunc (c %s) ApplyDefaults() %s {\n\tconfig := %s\n", configName, resolvedType, defaultExpr)
		for _, field := range structType.Fields.List {
			fieldName := field.Names[0].Name
			if !ast.IsExported(fieldName) {
				continue
			}
			fmt.Fprintf(&body, "\tconfig.%s = c.%s.ValueOr(config.%s)\n", fieldName, fieldName, fieldName)
		}
	...
	if presetFunc != "" {
		...
		fmt.Fprintf(&body, "\nfunc PresetConfig(level int) %s {\n\tpreset := %s(%s)\n\treturn %s{\n", configName, presetFunc, level, configName)
		for _, field := range structType.Fields.List {
			fieldName := field.Names[0].Name
			if !ast.IsExported(fieldName) {
				continue
			...
			fmt.Fprintf(&body, "\t\t%s: optional.Some(preset.%s),\n", fieldName, fieldName)
		}
		fmt.Fprintf(&body, "\t}\n}\n\nfunc WithPreset(level int) %s {\n\treturn func(c *%s) { *c = PresetConfig(level) }\n}\n", optionName, configName)
	}
