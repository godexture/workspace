...
func TestSpoolCapabilityAdaptationIsOnlyASequentialToRandomWriteFallback(t *testing.T) {
	...
	adapted, selected, ok := spoolCapabilities(sequential, access.NewRequirements(access.AnyOf(access.RandomWrite)))
	...
	if _, _, ok := spoolCapabilities(random, access.NewRequirements(access.AnyOf(access.RandomWrite))); ok {
		t.Fatal("direct random-write capability was treated as a spool candidate")
	}
	if _, _, ok := spoolCapabilities(sequential, access.NewRequirements(access.AnyOf(access.SequentialWrite))); ok {
